...
export class Raycast {
	...
	constructor(distance = 25.0, flag = -1, ignore = PlayerPedId()) {
		...
		const forwardCoords = cameraCoords.add(
			forwardVector.multiply(distance)
		);
	...
	public static getForwardVector(rotation: Vector3) {
		...
		return new Vector3(
			-Math.sin(rotationRadians.z) *
				Math.abs(Math.cos(rotationRadians.x)),
	...
	public getSurfaceRotation() {
		...
		const rotationAxis = upVector.crossProduct(
			this.surfaceNormal
		).normalize;
		const rotationAngle = Math.acos(
			upVector.dotProduct(this.surfaceNormal)
		);
